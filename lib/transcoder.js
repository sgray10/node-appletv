...
function TranscodePipe(media, port, position) {
  events.EventEmitter.call(this);
  var self = this;
  self.ffmpeg_path = path.join(
    path.dirname(fs.realpathSync(__filename)), '../deps/bin/ffmpeg'
  );
  self.segmenter_path = path.join(
    path.dirname(fs.realpathSync(__filename)), '../deps/bin/segmenter'
  );
...
TranscodePipe.prototype.startServer = function(port) {
  ...
  self.server.once('listening', function() {
    console.log('[transcoder]: segment consumer started on port', port);
  });
